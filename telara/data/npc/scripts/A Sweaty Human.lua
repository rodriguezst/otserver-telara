...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "yes") then
		...
		elseif npcHandler.topic[cid] == 2 then
			npcHandler:say("Me need gift for woman. We dance, so me want to give her bast skirt. But she big is. So I need many to make big one. Bring two okay? Me wait.", cid)
			npcHandler.topic[cid] = 0
		...
		elseif npcHandler.topic[cid] == 3 then
			if player:removeItem(3983, 2) then
	...
	elseif msgcontains(msg, "uth'kean") then
		...
		elseif player:getStorageValue(Storage.FriendsandTraders.TheSweatyCyclops) == 1 then
			npcHandler:say("Lil' one bring two bast skirts?", cid)
