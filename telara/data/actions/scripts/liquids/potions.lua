...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif itemId == strongHealthPot then
		...
		target:say("Aaaah...", TALKTYPE_MONSTER_SAY)
		item:remove(1)
		player:removeItem(strongEmptyPot, 1)
	...
	elseif itemId == greatSpiritPot then
		...
		target:say("Aaaah...", TALKTYPE_MONSTER_SAY)
		item:remove(1)
		player:removeItem(greatEmptyPot, 1)
...
player:addCondition(exhaust)
	...
	elseif itemId == supremeHealthPot then
		...
		if not doTargetCombatHealth(0, target, COMBAT_HEALING, 800, 1000, CONST_ME_MAGIC_BLUE) then
			return false
		end
...
player:addCondition(exhaust)
	...
	elseif itemId == ultimateSpiritPot then
		...
		if not doTargetCombatHealth(0, target, COMBAT_HEALING, 400, 550, CONST_ME_MAGIC_BLUE) or not doTargetCombatMana(0, target, 150, 250, CONST_ME_MAGIC_BLUE) then
			return false
		end
...
player:addCondition(exhaust)
	...
	elseif itemId == ultimateManaPot then
		...
		if not doTargetCombatMana(0, target, 400, 600, CONST_ME_MAGIC_BLUE) then
			return false
		end
