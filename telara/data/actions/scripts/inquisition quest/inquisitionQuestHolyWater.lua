local pos = {x=974,y=1079,z =7}
local summonPirate = {x=970,y=1080,z =7}
local time = 20
...
local function transformBack1()
	return (doTransformItem(getTileItemById( {x=220,y=1959,z =6},8754).uid, 8755))
end

local function transformBack2()
	return (doTransformItem(getTileItemById( {x=220,y=1959,z =6},8756).uid, 8757))
end

local function transformBack3()
	return (doTransformItem(getTileItemById( {x=220,y=1959,z =6},8758).uid, 8759))
end

local function transformBack4()
	return (doTransformItem(getTileItemById( {x=220,y=1959,z =6},8759).uid, 8753))
end

function onUse(cid, item, fromPosition, itemEx, toPosition)
	-- Eclipse
	if(itemEx.actionid == 3000) then
	...
	-- Haunted Ruin
	if(itemEx.actionid == 3003) then
		if(getPlayerStorageValue(cid, 200) == 12) then
			doSummonCreature("Pirate Ghost", summonPirate)
