...
[5] = {questStarted = 14007, questStorage = 14107, killsRequired = 1, raceName = "Crocodiles", repeatable = true, rewards = {{enable = true, type = "exp", values = 800}, {enable = true, type = "item", values = {3982,1}}, {enable = true, type = "money", values = 5000}, {enable = true, type = "points", values = 2}}},
...
if taskWindow:getId() == 1900 then
	...
	for i = 1, #tasks do
		if ((getPlayerStorageValue(cid, tasks[i].questStarted)) == 2 and (tasks[i].repeatable)) then
			task_status[i] = "[clear to repeat]"
			elseif ((getPlayerStorageValue(cid, tasks[i].questStarted)) == 2 and (tasks[i].repeatable == false)) then
	...
	taskWindow:setDefaultEnterButton(100)
    taskWindow:setDefaultEscapeButton(101)
