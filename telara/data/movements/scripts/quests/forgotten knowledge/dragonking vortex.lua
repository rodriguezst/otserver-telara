local function functionBack()
	local dragonking = Tile(Position(1890, 2057, 12)):getTopCreature()
	local soul, diference, health = false, 0, 0
	local spectators, spectator = Game.getSpectators(Position(1891, 2061, 11), false, false, 15, 15, 15, 15)
	...
	local specs, spec = Game.getSpectators(Position(1891, 2061, 11), false, false, 15, 15, 15, 15)
	for i = 1, #specs do
		spec = specs[i]
		if spec:isPlayer() then
			spec:teleportTo(Position(1891, 2061, 10))
		elseif spec:isMonster() and spec:getName():lower() == 'soul of dragonking zyrtarch' then
			spec:teleportTo(Position(1890, 2057, 12))
			health = spec:getHealth()
			diference = dragonking:getHealth() - health
		end
	end
	dragonking:addHealth( - diference)
	dragonking:teleportTo(Position(1891, 2061, 10))
end
...
function onStepIn(creature, item, position, fromPosition)
	...
	if creature:isPlayer() then
		creature:teleportTo(Position(1890, 2061, 11))
	end
	if creature:getName():lower() == 'dragonking zyrtarch' then
		local soul = Tile(Position(1890, 2057, 12)):getTopCreature()
		creature:teleportTo(Position(1890, 2057, 12))
		soul:teleportTo(Position(1891, 2061, 11))
